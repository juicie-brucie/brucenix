{
    ...
    outputs = inputs@{ self, ...}:
      let
       # ---- SYSTEM SETTINGS ---- #
        systemSettings = {
          system = "x86_64-linux"; # sys architecture
          hostname = "brucenix";
      ...
      # ---- USER SETTINGS ---- #
        userSettings = rec {
          username = "bruce";
          name = "bruce";
          email = "brucemackay17@proton.me";
          dotfilesDir = "~/.dotfiles";
          theme = "io";
          wm = "hyprland";
          wmType = if ((wm == "hyprland") || (wm == "plasma")) then "wayland" else "x11";
          browser = "firefox";
          #spawnBrowser = if ((browser )) #fore qutebrowser
          defaultRoamDir = "personal.p"; #?
          term = "alacritty"; #default terminal
          font = "jetbrains-mono";
          fontPpk = pkgs.jetbrains-mono;
          editor = "neovide";
        ...
        pkgs = (if ((systemSettings.profile == "homelab") || (systemSettings.profile == "server"))
            then
                pkgs-stable
            else
        ...
        # ---- LIB CONFIG ---- #
        # use stable if homelab or server
        # otherwise uses patched nixos-unstable
        lib = (if ((systemSettings.profile == "homelab") || (systemSettings.profile == "server"))
                then
                    inputs.nixpkgs-stable.lib
                else
                    inputs.nixpkgs.lib);

        # use stable if homelab or server otherwise use unstable
        home-manager = (if ((systemSettings.profile == "homelab") || (systemSettings.profile == "server"))
            then
